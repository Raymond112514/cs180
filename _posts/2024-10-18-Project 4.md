...
In the previous section, we need to manually label keypoints for both images. In this section, we automatically look for interesting keypoints in an image and perform warping. The overall step includes: 
1. keypoint finding
...
To find the keypoints, we used the Harris corner detector. The idea is that corners in an image exhibits strong signal when the local patch is shifted slightly. The score at each pixel is computed as 

Some examples of corners detected are shown below.
